...
func (c *Cart) GetCart(db *gorm.DB,cartID string)(*Cart, error){
	...
	err = db.Debug().Preload("CartItems").Preload("CartItems.Product").Model(Cart{}).Where("id = ?",cartID).First(&cart).Error
	if err != nil {
		return nil,err
	}
	return &cart,nil
...
func (c *Cart) CalculateCart(db *gorm.DB, cartID string)(*Cart,error){
	...
	err:= db.Debug().First(&cart,"id = ?", cartID).Error
	if err!= nil{
        return nil,err
    }

	cart.BaseTotalPrice = decimal.NewFromFloat(cartBaseTotalPrice)
	cart.TaxAmount = decimal.NewFromFloat(cartTaxAmount)
	cart.DiscountAmount = decimal.NewFromFloat(cartDiscountAmount)
	cart.GrandTotal = decimal.NewFromFloat(cartGrandTotal)

	err= db.Debug().Save(&cart).Error
	if err != nil{
		return nil,err
	}
...
func (c *Cart) AddItem(db *gorm.DB, item CartItem)(*CartItem,error){
	...
	err= db.Debug().Model(CartItem{}).Where("id = ?", existItem.ID).Updates(existItem).Error
	if err!= nil {
        return nil,err
    }
...
func (c *Cart) UpdateItemQty(db *gorm.DB, itemID string, qty int) (*CartItem,error){
    ...
    basePrice,_:= item.BasePrice.Float64()
    taxAmount := GetTaxAmount(basePrice)
    discountAmount:=0.0

    item.Qty = qty
    item.BaseTotal = decimal.NewFromFloat(basePrice * float64(item.Qty))
    item.SubTotal = decimal.NewFromFloat(float64(item.Qty) * (basePrice + taxAmount - discountAmount))

    err= db.Debug().Model(CartItem{}).Where("id = ?", item.ID).Updates(item).Error
    if err!= nil {
        return nil,err
    }
    return &item, nil
