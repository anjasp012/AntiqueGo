...
func (s *Server) Midtrans(w http.ResponseWriter, r *http.Request) {
	...
	order, err := orderModel.FindByID(s.DB, paymentNotification.OrderID)
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusForbidden)
	...
	_, err = paymentModel.CreatePayment(s.DB, &models.Payment{
		OrderID:           order.ID,
		Amount:            amount,
...
func isPaymentSuccess(payload *models.MidtransNotification) bool {
	...
	if payload.PaymentType == string(snap.PaymentTypeCreditCard) {
		paymentStatus = (payload.TransactionStatus == consists.PaymentStatusCapture) && (payload.FraudStatus == consists.FraudStatusAccept)
	} else {
		paymentStatus = (payload.TransactionStatus == consists.PaymentStatusSettlement) && (payload.FraudStatus == consists.FraudStatusAccept)
	}
